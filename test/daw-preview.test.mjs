...
const dawDir = new URL("../src/features/daw-preview/", import.meta.url);
...
test("knob atlases are vendored, downscaled, and referenced by public path", () => {
  const assets = read("src/features/daw-preview/vendor/NAMRackControlAssets.ts");
...
test("NAM design artwork is vendored, downscaled, and served from public/", () => {
  ...
  const assets = read("src/features/daw-preview/vendor/NAMDesignAssets.ts");
  assert.doesNotMatch(assets, /import\.meta/);
  assert.match(assets, /\/assets\/openstudio\/nam\/design\//);
  assert.match(read("src/features/daw-preview/vendor/NAMRackDesignPort.tsx"), /"\/assets\/openstudio\/nam\/rack-studio-backdrop-v2\.webp"/);
});
...
test("the stage driver respects reduced motion, pauses off-screen, and loads GSAP late", () => {
  const driver = read("src/features/daw-preview/stage/useStageTimeline.ts");
  ...
  assert.match(read("src/features/daw-preview/useSessionTimeline.ts"), /from "\.\/stage\/useStageTimeline"/);
  assert.match(read("src/features/daw-preview/stage/StageFrame.tsx"), /daw-session--showcase/);
