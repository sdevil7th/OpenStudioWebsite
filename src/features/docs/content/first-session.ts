...
const doc: DocContent = {
  ...
  blocks: [
    {
      type: "p",
      text: `One continuous session: a guitar part through the NAM Rack, a MIDI drum part, two edits, a quick mix, and a rendered file. It assumes you have finished [Getting started](${SITE_PATHS.docs}/getting-started) and can hear playback. Shortcuts are the OpenStudio default keyboard profile; \`Ctrl\` means \`Cmd\` on macOS.`,
    },
    ...
    },
    {
      ...
      tone: "note",
      label: "Drum editor",
      text: `The Drum Editor action exists, but a mounted drum-grid editor is not in the current build. Use the Piano Roll for drum notes. More in [MIDI & piano roll](${SITE_PATHS.docs}/midi-and-piano-roll).`,
    },
    ...
    {
      type: "p",
      text: `Sends, the routing matrix, and automation are in [Mixing & routing](${SITE_PATHS.docs}/mixing-and-routing).`,
    },
    ...
    },
    {
      ...
      tone: "warn",
      label: "MP3 and OGG",
      text: `Those formats go through FFmpeg, bundled on Windows only. On macOS and Linux you need a system \`ffmpeg\` on your \`PATH\`. Stems, regions, and DDP are in [Rendering & export](${SITE_PATHS.docs}/rendering-and-export).`,
    },
    ...
    { type: "h2", id: "where-next", text: "Where next" },
    {
      type: "ul",
      items: [
        `[Audio setup](${SITE_PATHS.docs}/audio-setup) if monitoring felt late or you heard crackle.`,
        `[Plugins & scanning](${SITE_PATHS.docs}/plugins-and-scanning) if an instrument did not show up in the browser.`,
        `[Keyboard shortcuts](${SITE_PATHS.docs}/keyboard-shortcuts) to switch to a profile that matches the DAW you came from.`,
        `[Troubleshooting](${SITE_PATHS.docs}/troubleshooting) for anything that did not behave as described here.`,
      ],
    },
  ],
};
