 ...
 *   **bold**   `code`   [label](url)
 *
 * Internal links use canonical paths (`/docs/audio-setup`), external ones full URLs.
 */
...
export interface DocMeta {
  ...
  summary: string;
  group: DocGroupId;
  /** Upstream file this page was written from — surfaces as "Source on GitHub". */
  sourceUrl: string;
}
...
export interface DocContent {
  ...
  updated: string;
  /** Explicit source verification; never inferred from whichever release is newest. */
  appReference: { commit: string; channel: "development" | "release"; version?: string };
  blocks: DocBlock[];
}
