...
import type { SpReveal } from "@/hooks/useSpReveal";
...
export const IconGradientDefs = () => (
  <svg className="absolute" aria-hidden="true" focusable="false" height="0" width="0">
    <defs>
...
const EYEBROW_TONES = {
  accent: "",
  teal: "sp-eyebrow--teal",
  warn: "sp-eyebrow--warn",
  good: "sp-eyebrow--good",
} satisfies Record<NonNullable<EyebrowProps["tone"]>, string>;

export const Eyebrow = ({ children, icon: Icon, tone = "accent", className = "" }: EyebrowProps) => (
  <div className={`sp-eyebrow ${EYEBROW_TONES[tone]} ${className}`.trim()}>
...
export const Kicker = ({ children, className = "" }: { children: ReactNode; className?: string }) => (
  <div className={`sp-kicker ${className}`}>{children}</div>
);

type LinkDestination = { to: string; href?: never } | { href: string; to?: never };

type CtaProps = LinkDestination & {
  children: ReactNode;
  icon?: ComponentType<LucideProps>;
 ...
 * glow in site.css originates under the cursor.
 */
...
type ArrowLinkProps = LinkDestination & {
  children: ReactNode;
...
export const ArrowLink = ({ children, to, href, tone = "accent", className = "" }: ArrowLinkProps) => {
  const classes =
    `sp-link${tone === "teal" ? " sp-link--teal" : tone === "plain" ? " sp-link--plain" : ""} ${className}`.trim();
  const content = (
    <>
      {children}
...
export const WarnCallout = ({ children, label }: { children: ReactNode; label: string }) => (
  <div className="sp-callout-warn">
    <div className="sp-callout-label text-[var(--sp-warn)]">
      <TriangleAlert aria-hidden="true" size={13} strokeWidth={1.8} />
      {label}
    </div>
    <p className="sp-body text-[13.5px] leading-[1.65]">{children}</p>
  </div>
);

export const HonestCallout = ({ children }: { children: ReactNode }) => (
  <div className="sp-callout-honest">
    <div className="sp-callout-label text-[var(--sp-mono-muted)]">
      <ShieldCheck aria-hidden="true" size={13} strokeWidth={1.8} />
      Worth knowing
    </div>
    <p className="sp-body text-[13.5px] leading-[1.7]">{children}</p>
  </div>
);
...
/** Anchor that stays inside the router for internal paths and opens a new tab for external URLs. */
...
export const NoteCallout = ({ children, label }: { children: ReactNode; label: string }) => (
  <div className="sp-callout-honest">
    <div className="sp-callout-label text-[var(--sp-accent)]">
...
export const DownloadCta = ({
  ...
  if (direct && info) {
    return (
      <Cta
        className={className}
        href={platform ? (release.platforms[platform].directUrl ?? info.href) : info.href}
        icon={Download}
        variant={variant}
      >
        {text}
  ...
  return (
    <Cta className={className} icon={Download} to={SITE_PATHS.download} variant={variant}>
      {text}
    </Cta>
  );
};
