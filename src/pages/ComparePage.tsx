...
import { Cta, DownloadCta, Eyebrow, HonestCallout, Kicker } from "@/components/ui/primitives";
...
const ROWS: { label: string; cells: [Cell, Cell, Cell, Cell, Cell, Cell] }[] = [
  {
    ...
    cells: [
      included(),
      paid("Paid"),
      limited("Free / 8 tracks"),
      limited("60-day eval"),
      limited("Browser Studio / paid"),
      none(),
    ],
  },
  {
    ...
    cells: [
      ...
      limited("Included in eval"),
      limited("Export only / Premier"),
      none(),
    ],
  },
  {
    ...
    cells: [
      included(),
      paid("VariAudio / paid"),
      paid("Melodyne separate"),
      limited("Basic ReaTune"),
      limited("Basic Studio tools / paid"),
      none(),
    ],
  },
  {
    ...
    cells: [
      included("Free / optional"),
      none(),
      none(),
      none(),
      limited("Free / credit-limited"),
      limited("Free / credit-limited"),
    ],
  },
  {
    ...
    <>
      ...
      <div className="sp-container pt-[64px]" data-sp-reveal="hero">
        ...
        <p className="sp-lede max-w-[680px]">
          Music creation has split into two workflows: legacy DAWs with professional control behind paywalls, and AI
          generators with instant ideas and shallow production. OpenStudio wraps the whole path from creation to
      ...
      <div className="sp-container pt-[38px]">
        <div className="sp-grid-3" data-sp-reveal="stagger">
          {PARADIGMS.map((paradigm) => (
            <div key={paradigm.title} className="sp-card p-[26px_28px]">
              ...
              <div className="flex flex-col gap-[10px]">
                <div className="flex gap-[10px]">
                  <span className="text-[var(--sp-good)] pt-[2px]">
                    <Check aria-hidden="true" size={14} strokeWidth={2.2} />
                  </span>
                  <p className="sp-body text-[13.5px] leading-[1.55]">{paradigm.pro}</p>
                </div>
                <div className="flex gap-[10px]">
                  <span className="text-[var(--sp-warn)] pt-[2px]">
                    <Minus aria-hidden="true" size={14} strokeWidth={2.2} />
                  </span>
                  <p className="sp-body text-[13.5px] leading-[1.55]">{paradigm.con}</p>
                </div>
              </div>
            </div>
          ))}
          <div className="sp-card sp-card--dark p-[26px_28px]">
            <Eyebrow icon={Zap} tone="teal">
              The gap
            </Eyebrow>
            <div className="[font:700_20px/1.2_'Space_Grotesk',_sans-serif] tracking-[-0.02em] mb-[12px]">
              One tool for the full creation-to-production workflow. Free.
            </div>
            <p className="sp-body text-[13.5px] text-[var(--sp-dark-body)]">
              Multitrack recording, MIDI and graphical pitch editing, VST3 / CLAP / LV2 / ARA2 hosting, local AI
              generation, variation, and stem separation, and a NAM guitar rig with pedals, cab, EQ, and effects, all in
              the same project with no paid edition.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="sp-container pt-[44px]">
        ...
        <div className="sp-card sp-card--tight sp-scroll-x" data-sp-reveal="rise">
          <div className="grid [grid-template-columns:1.2fr_repeat(6,_1fr)] min-w-[1060px]">
            <div className="sp-matrix__sticky" style={cellStyle({ header: true })} />
            {COLUMNS.map((column, index) => (
              <div
                key={column}
                className={`[font:700_14px/1.3_'Space_Grotesk',_sans-serif] ${index === 0 ? "sp-compare__cell--us" : undefined}`}
                style={{ ...cellStyle({ header: true }), color: index === 0 ? "var(--sp-accent)" : "var(--sp-ink)" }}
              >
                {column}
              </div>
            ))}
            {ROWS.map((row) => (
              <Fragment key={row.label}>
                <div
                  className="sp-matrix__sticky [font:600_13px/1.4_'Space_Grotesk',_sans-serif]"
                  style={{ ...cellStyle({}) }}
                >
                  {row.label}
                </div>
        ...
        <div className="sp-compare__legend sp-mono">
          <span className="text-[var(--sp-good)]">
            <Check aria-hidden="true" size={12} strokeWidth={2.2} /> included in the free build
          </span>
          <span className="text-[var(--sp-warn)]">paid edition or separate purchase</span>
          <span>limited, trial, or credit-gated</span>
          <span className="text-[var(--sp-mono-muted)]">
      ...
      {/* Cost illustration */}
      <div className="sp-container pt-[44px]">
        <div className="sp-row min-[901px]:grid-cols-[1fr_1fr] gap-[36px]" data-sp-reveal="stagger">
          <div>
            <Kicker>The first-year studio</Kicker>
            <h2 className="sp-h2 text-[30px]">$0, against an illustrative ~$830 stack.</h2>
            <p className="sp-body max-w-[460px] mb-[14px]">
              To cover the same ground with separate products you would typically buy a DAW, a pitch editor, a guitar
              rig, and a generation subscription. OpenStudio ships all four in the base app.
            </p>
            <p className="sp-mono m-[0px] leading-[1.6]">
              List-price illustration only; not a claim of feature or quality equivalence.
            </p>
          </div>
          <div className="sp-card sp-card--tight overflow-hidden">
            {STACK.map(([role, product], index) => (
              <div
                className="grid [grid-template-columns:1fr_1fr_1fr] gap-[12px] p-[13px_18px] [font:400_13px/1.5_'Space_Grotesk',_sans-serif] text-[var(--sp-body)]"
                key={role}
                style={{ borderTop: index > 0 ? "1px solid var(--sp-hairline)" : undefined }}
              >
                <span className="font-[600] text-[var(--sp-ink)]">{role}</span>
                <span>{product}</span>
                <span className="text-[var(--sp-good)] font-[600] text-right">OpenStudio · $0</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Sources */}
      <div className="sp-container pt-[30px]" data-sp-reveal="rise">
        <div className="sp-mono leading-[1.8]">
          Sources:{" "}
      ...
      {/* CTA */}
      <div className="sp-container pt-[44px] pb-[62px] flex gap-[14px] flex-wrap" data-sp-reveal="stagger">
        <DownloadCta />
        <Cta icon={Scale} to={SITE_PATHS.features} variant="outline">
          Every feature
        </Cta>
      </div>
    </>
